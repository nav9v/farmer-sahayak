...
import { ArrowLeft, BookOpen, Volume2, Play, Pause } from "lucide-react";
    ...
    <div className={`fixed inset-0 flex flex-col bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 transition-transform duration-300 ${isExiting ? 'translate-x-full' : 'translate-x-0'}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative z-10">
        <header className="px-3 pt-3">
          <div className="bg-cream-50/70 backdrop-blur-2xl rounded-3xl border border-yellow-100/50 px-4 md:px-6 py-3 md:py-4 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <ArrowLeft className="w-6 h-6 text-amber-700 relative z-10" />
              </button>
              ...
              <div className="flex-1">
                <h2 className="text-lg md:text-xl font-bold text-gray-800">{t('learn')}</h2>
                <p className="text-xs text-gray-600">{t('farmingTips')}</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          <div className="space-y-4 max-w-4xl mx-auto">
            {tips.map((tip, index) => (
              <div
                key={index}
                className={`group relative bg-gradient-to-br from-amber-50/80 to-orange-50/80 backdrop-blur-sm rounded-3xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] ${
                  selectedTip === index 
              ...
              >
                ...
                <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r transition-all duration-300 ${
                  selectedTip === index
                    ...
                    : 'from-amber-300 via-yellow-300 to-orange-300 opacity-0 group-hover:opacity-100'
                }`} />
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-start gap-3 mb-3">
                        <div className="mt-1 p-2 bg-gradient-to-br from-amber-100 to-yellow-100 rounded-xl">
                          <BookOpen className="w-5 h-5 text-amber-700" />
                        </div>
                        <h3 className="flex-1 text-lg md:text-xl font-bold text-gray-800 leading-tight">{tip.title}</h3>
                      </div>
                      <p className="text-sm md:text-base text-gray-600 leading-relaxed pl-14">{tip.content}</p>
                    </div>
                    <button
                      onClick={() => handlePlayTip(index)}
                      className={`shrink-0 p-4 md:p-5 rounded-2xl transition-all duration-300 shadow-lg hover:shadow-xl ${
                        isPlaying && selectedTip === index
                          ? 'bg-gradient-to-br from-red-500 to-rose-600 hover:from-red-600 hover:to-rose-700 scale-110'
                          : 'bg-gradient-to-br from-amber-500 via-yellow-500 to-orange-600 hover:from-amber-600 hover:via-yellow-600 hover:to-orange-700'
                      } text-white`}
                      aria-label={isPlaying && selectedTip === index ? 'Pause' : 'Play'}
                    >
                      {isPlaying && selectedTip === index ? (
                        <Pause className="w-5 h-5 md:w-6 md:h-6" />
                      ) : (
                        <Play className="w-5 h-5 md:w-6 md:h-6 translate-x-0.5" />
                      )}
                    </button>
                  </div>
                </div>
                
                {/* Bottom decorative line when selected */}
                {selectedTip === index && (
